...
def set_area(face_key_point):
    ...
    area = {1: left_brow, 2: right_brow, 3: nose, 4: mouth, 5: left_brow1, 6: right_brow1, 7: left_eye_below,
            8: right_eye_below}
    return area
...
def solve(path1, path2):
    ...
    for i in range(total_frame):
        ...
        if count == 1:
            ...
            ROI.append(temp_ROI)
            cv2.imwrite(path2 + '/' + str(count) + '.jpg',temp_ROI)
        else:
            # 第二帧开始就要进行仿射变换矩阵的计算
...
def mainProcess(path1, path2):
    p0, frame, ROI = solve(path1, path2)
...
for i in range(len(remain_dir)):
    ...
    print(path1)
    print(path2)
    base_path = 'E:/ZLW_ROI/' + remain_dir[i]
    print(base_path)
