...
def generate_ROI(video_path, save_path ,list):
    ...
    for i in range(1, total_frame+1):
        ...
        if i == 1:
            ...
            area = set_area(p0)
            first_ROI_frame = extractROI_chg(area, list, first_frame)
            ROI_frame.append(first_ROI_frame)
            temp_write_path = save_path + '/' + 'point.txt'
            with open(temp_write_path, 'w') as f:
                f.write(str(p0))
            cv2.imwrite(save_path + '/' + str(i) + '.jpg', ROI_frame[i-1])
        else:
            ret, temp_frame = cap.read()
            temp_frame = solve(first_frame, temp_frame, False, p0, feature_point_M_one)

            frame.append(temp_frame)
            temp_ROI = extractROI_chg(area, list, temp_frame)
            ROI_frame.append(temp_ROI)
            cv2.imwrite(save_path + '/' + str(i) + '.jpg', ROI_frame[i-1])

def get_path(target_path, area, remain_dir):
    area = str(area)
    path1 = 'E:/CAS(ME)^2/rawvideo/' + remain_dir + '/'
    path2 = target_path + area + '/' + remain_dir + '/'
    path_list1 = []
    path_list2 = []
    #counnt=1表示进入到第一个目录下
    count = 1
...
remain_dir = ['s15']
target_path = 'E:/ZLW_data/'
if not os.path.exists(target_path):
    print('创建' + target_path)
    os.mkdir(target_path)
for i in range(len(area)):
    temp_path = target_path + str(area[i])
    if not os.path.exists(temp_path):
        print('创建' + temp_path)
        os.mkdir(temp_path)
    for j in range(len(remain_dir)):
        temp_path1 = temp_path + '/' + remain_dir[j]
        if not os.path.exists(temp_path1):
            print('创建' + temp_path1)
            os.mkdir(temp_path1)
        path1, path2 = get_path(target_path, area[i], remain_dir[j])
        for k in range(len(path1)):
            if not os.path.exists(path2[k]):
                print('创建:' + path2[k])
                os.mkdir(path2[k])
                generate_ROI(path1[k], path2[k], area[i])

#运行另一个文件
path = 'python ' + 'D:/ZLW/ZLW_lab/ZLW/ROI的模相减1.0.py'
os.system(path)
