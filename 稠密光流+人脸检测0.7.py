...
for i in range(1, add_flow_number+1):
    if i == 1:
        ...
        p0, initial_frame = pre_process(None, temp_frame, True, None)
        frame.append(initial_frame)
        #再读step个帧
        for j in range(step):
            ret, temp_frame = cap.read()
            temp_frame = pre_process(initial_frame, temp_frame, False, p0)
        ...
        cv2.imwrite('E:/flow1/' + str(i) + '.jpg', bgr)
    else:
        #计算第二个以后的光流累加
        ret, temp_frame = cap.read()
        temp_frame = pre_process(initial_frame, temp_frame, False, p0)
        ...
        cv2.imwrite('E:/save_flow/' + str(i) + '.jpg', bgr)
        #show_gray_hist(bgr)

    #if imshow('flow', frame[i-1]) == 27:
    #    break
